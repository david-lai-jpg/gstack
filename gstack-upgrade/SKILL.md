...
Upgrade gstack to the latest version and show what's new. This is manual only:
no skill preamble checks for updates, and no session-start background updater exists.
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/gstack/gstack.git "$TMP_DIR/gstack"
...
When invoked directly as `/gstack-upgrade`:

1. Run the Step 1 bash block above to detect the primary install type and directory (`INSTALL_TYPE` and `INSTALL_DIR`).
2. Run the normal upgrade steps above (`git fetch`, backup, pull/clone, setup, migration notes, changelog summary).
3. Check for a stale local vendored copy using the Step 4.5 detection bash block (`LOCAL_GSTACK`) and team mode status (`TEAM_MODE`).

**If `LOCAL_GSTACK` is empty** (no local vendored copy): tell the user "Upgrade complete (v{version})."
