 ...
 * preferences, repo mode detection, model overlays, and completion rules.
 */
...
export function generatePreamble(ctx: TemplateContext): string {
  ...
  const sections = [
    ...
    // Plan-mode-skill semantics at position 1: after bash (so branch and
    // repo context are live) and before all onboarding gates so
